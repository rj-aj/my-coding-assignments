...
WHERE actor_id = 15 AND special_features LIKE '%Behind the Scenes%' AND rating='G';
...
SELECT film.film_id, title, description, release_year, special_features, category.name AS genre, rental_rate
FROM film
JOIN film_category
...
SELECT actor.actor_id, title,CONCAT(actor.first_name, ' ', actor.last_name) AS actor_name,
film.film_id,description, release_year,rating,special_features,category.name AS genre
FROM film
JOIN film_actor
ON film.film_id = film_actor.film_id
JOIN actor
